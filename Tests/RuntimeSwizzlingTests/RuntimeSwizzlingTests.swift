...
//func removeViewDidAppearSwizzle(_ class_: AnyClass, originalImplementation: IMP?) {
//	let selector = #selector(UIViewController.viewDidAppear(_:))
...
//				removeViewDidAppearSwizzle(class_, originalImplementation: implementation)
	...
	implementation = _swizzleViewDidAppear(class_, to: swizzledBlock)
}
...
final class RuntimeSwizzlingTests: XCTestCase {
	func test() {
		...
		swizzleViewDidAppear(viewController: sut) { `self`, animated in
